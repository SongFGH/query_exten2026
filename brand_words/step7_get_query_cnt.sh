...
# 示例：sh step7_get_query_cnt.sh "/data3/longcanwu/accuracy_word_first_location/online_toufang_20260120/brand_words/data" "hdfs://bj04-region06/region06/17200/app/develop/longcanwu/accuracy_word_first_location/online_toufang_20260120/brand_words/data"
...
if [ -z "${hdfs_project_data_path}" ]; then
    ...
    echo "===== 用法示例：sh step7_get_query_cnt.sh /data3/local_dir hdfs://xxx/xxx ====="
    exit 1
fi
    ...
    spark3-sql \
        ...
        -e "
           -- 关键2：关闭所有压缩相关配置，确保明文不被压缩
            ...
            INSERT OVERWRITE DIRECTORY '${hdfs_project_data_path}/step7_get_query_cnt'
            ...
            -- 执行查询（与原逻辑一致）
            SELECT keyword, cnt 
            FROM ads_strategy_adnet.dw_response_analysis_query_last_month_cnt_d
            WHERE 1 > 0
            -- and day = '2026-01-20'
...
echo "===== 数据将保存到HDFS路径：${hdfs_project_data_path}/step7_get_query_cnt ====="
obtain_data

# 6. 验证HDFS路径是否生成数据（可选，增加脚本健壮性）
hdfs dfs -test -d "${hdfs_project_data_path}/step7_get_query_cnt"
if [ $? -eq 0 ]; then
