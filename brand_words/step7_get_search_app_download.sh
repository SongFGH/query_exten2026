...
# 用法：sh step7_get_search_app_download.sh [本地路径] [HDFS路径]
# 示例：sh step7_get_search_app_download.sh "/data3/longcanwu/accuracy_word_first_location/online_toufang_20260120/brand_words/data" "hdfs://bj04-region06/region06/17200/app/develop/longcanwu/accuracy_word_first_location/online_toufang_20260120/brand_words/data"
...
function obtain_data()
{
    spark3-sql \
        ...
        -e "
            ...
            INSERT OVERWRITE DIRECTORY '${hdfs_project_data_path}/step7_get_search_app_download'
            ...
            FROM comsearch.da_appsearch_search_app_download_ratio_ds
            where 1 > 0
            -- and day = '2026-01-20'
...
echo "===== 数据将保存到HDFS路径：${hdfs_project_data_path}/step7_get_search_app_download ====="
obtain_data

# 6. 验证HDFS路径是否生成数据（可选，增加脚本健壮性）
hdfs dfs -test -d "${hdfs_project_data_path}/step7_get_search_app_download"
if [ $? -eq 0 ]; then
